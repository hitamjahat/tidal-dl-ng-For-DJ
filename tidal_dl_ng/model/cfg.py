...
class Settings:
    ...
    api_rate_limit_batch_size: int = (
        20  # Number of albums to process before applying rate limit delay
    )
...
class HelpSettings:
    ...
    use_primary_album_artist: str = (
        "Use only the primary album artist for folder paths instead of track artists."
    )
    lyrics_file: str = (
    ...
    format_playlist: str = (
        "Where to download playlists and how to name the items."
    )
    ...
    extract_flac: str = (
        "Extract FLAC audio tracks from MP4 containers and save them as `*.flac` (uses FFmpeg)."
    )
    ...
    download_delay_sec_min: str = (
        "Lower boundary for the calculation of the download delay in seconds."
    )
    download_delay_sec_max: str = (
        "Upper boundary for the calculation of the download delay in seconds."
    )
